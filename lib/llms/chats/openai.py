...
class BaseChatOpenAI(ChatLLM):
    ...
    def openai_response(self, client: OpenAI, **kwargs):
        """Get the openai response"""
        raise NotImplementedError
    
    async def aopenai_response(self, client: AsyncOpenAI, **kwargs):
        """Get the async openai response"""
        raise NotImplementedError
    ...
    async def ainvoke(
        ...
        input_messages = self.prepare_message(messages)
        resp = await self.aopenai_response(
            client, messages=input_messages, stream=False, **kwargs
        )

        parsed_resp = resp.dict()

        return self.prepare_output(parsed_resp)
...
class StructuredOutputChatOpenAI(BaseChatOpenAI):
    ...
    async def aopenai_response(self, client: AsyncOpenAI, **kwargs):
        ...
        res = await client.beta.chat.completions.parse(**params)
