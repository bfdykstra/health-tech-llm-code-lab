...
from lib.storages import BaseDocumentStore, BaseVectorStore
...
class VectorRetrieval(BaseRetrieval):
    ...
    def get_embedding(self, text):
        print('getting embedding... ', len(text))
        return self.embedding(text)[0].embedding
    ...
    def run(
        ...
        if self.retrieval_mode == "vector":
            if type(text) == 'Document':
                emb = self.get_embedding(text.text)
            else:
                emb = self.get_embedding(text)
            # _, scores, ids = self.vector_store.query(
            #     embedding=emb, top_k=top_k_first_round, **kwargs,
            # )
            # print('without bm25: ', scores, ids)
            _, scores, ids = self.vector_store.query(
                embedding=emb, top_k=top_k_first_round, **kwargs, alpha=0, mode='hybrid' #use bm25 with hybrid retrieval?
            )

            docs = self.doc_store.get(ids)
            print(f"Got {len(docs)} from vectorstore")
            result = [
