...
from lib.storages.docstores.base import BaseDocumentStore
...
class LanceDBDocumentStore(BaseDocumentStore):
    ...
    def query(
        ...
        # stripped_query = re.sub(r'[\+\-\&\|\!\(\)\{\}\[\]\^\"\~\*\?\:\\\/]', r'\\\g<0>', query)
        stripped_query = query
        try:
            document_collection = self.db_connection.open_table(self.collection_name)
            if query_filter:
                docs = (
                    document_collection.search(stripped_query, query_type="fts")
            ...
            print('doc store query threw Syntax error')
            print(e)
            docs = []
        return [
            Document(
    ...
    def count(self) -> int:
        ...
        return len(document_collection.to_pandas())

    def get_all(self) -> List[Document]:
        document_collection = self.db_connection.open_table(self.collection_name)

        document_collection.search().to_list().limit(MAX_DOCS_TO_GET)
